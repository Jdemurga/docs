...
| **Sovereign Guide** | `#guideSection` (in `index.html`) + `knowledgeBaseEs`/`knowledgeBaseEn` (in `app.js`) | In-terminal knowledge base localized via `localStorage.vexel_lang`. See [Sovereign Guide](/sovereign-guide). |
| **Edge proxy** | `nginx.conf` | Reference Nginx configuration that serves the static bundle and reverse-proxies `/api`, `/brain`, and the Socket.io upgrade to Vexel Core and Brain. |
...
- [Sovereign Guide](/sovereign-guide) — in-terminal knowledge base surfaced from `#guideSection`
